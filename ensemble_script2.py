...
for rand_st in rand_state:
    # Split Train/Test Layer 1
    data_train, data_test, labels_train, labels_test, new_mat_train, new_mat_test = train_test_split(all_train_cleaned, Class, new_mat, test_size = layer1_ratio, random_state  = rand_st)
    ...
    mean_acc_basicScore.append(basic_scorefi1)
    
    # Score Final
    best_score, best_model = second_layer(advced_model, proba_pca_train, proba_pca_test, labels_train,labels_test)
    mean_acc_ensemb.append(best_score)
    
print " \n CV basic mean score: ", np.mean(mean_acc_basicScore), " +-",np.sqrt(np.var(mean_acc_basicScore))
print " \n CV ensemble mean score: ", np.mean(mean_acc_ensemb), " +-",np.sqrt(np.var(mean_acc_ensemb))
